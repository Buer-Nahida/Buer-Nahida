...
void PrintAll(const std::unordered_map<UD_MapKey1, UD_MapKey2, UD_MapKey3> &mapName,
              const char *sepChar = " ") {
...
void PrintAll(const std::map<MapKey1, MapKey2, MapKey3> &mapName, const char *sepChar = " ") {
    for (const auto &arrVar : mapName) {
        std::cout << arrVar.first << sepChar << arrVar.second << std::endl;
    }
}

template <typename ArrType> void PrintAll(const ArrType &arrName, const char *sepChar = " ") {
